        ...
        <div className="container mx-auto">
          <div className="relative rounded-3xl overflow-hidden bg-slate-900 shadow-2xl h-[360px] md:h-[420px] flex items-center group cursor-pointer">
            ...
            <div className="relative z-10 p-8 md:p-16 max-w-2xl">
              ...
              >
                Combine your favorite local purchases into one unified cart, delivered securely at light speed.
              </motion.p>
            ...
            {stores.map((store) => {
              ...
              return (
                <div key={store.id} 
                  ...
                  onClick={() => setSearchParams({ search: store.name })}
                >
      ...
      <section className="py-12 md:py-16">
        <div className="container mx-auto px-4">
          <div className="flex flex-col mb-8">
            <div className="flex items-center justify-between mb-4">
              ...
              {searchFilter && (
                <button 
                  ...
                  className="text-sm font-semibold text-slate-500 hover:text-slate-900 bg-white px-4 py-2 border border-slate-200 rounded-lg shadow-sm transition-all"
                >
                  Reset Filters
                </button>
              )}
            ...
            {searchFilter && (
              <div className="flex items-center justify-between bg-blue-50 border border-blue-100/50 rounded-2xl px-5 py-3 text-sm">
                <span className="text-slate-700">
                  Showing results for <span className="font-bold text-blue-600">"{searchFilter}"</span> ({filteredProducts.length} items found)
                </span>
                <button 
                  onClick={() => setSearchParams({})}
          ...
          {filteredProducts.length === 0 ? (
            <div className="bg-white rounded-3xl border border-slate-200 py-16 px-4 text-center">
              <ShoppingBag size={48} className="mx-auto text-slate-300 mb-4" />
              <h3 className="text-lg font-bold text-slate-900 mb-1">No products found</h3>
              <p className="text-slate-500 max-w-md mx-auto text-sm">We couldn't find any products matching "{searchFilter}". Try searching for something else or reset your filter.</p>
              <button 
                onClick={() => setSearchParams({})}
                className="mt-6 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2.5 px-6 rounded-xl text-sm transition-all"
              >
                Show All Products
              </button>
            </div>
